  ...
  GPUNodeStack *outputs)
{
  ...
  filtered_inputs[9] = (float)node->custom3;
  filtered_inputs[10] = blender::float4x4(scene_linear_to_working);
  filtered_inputs[11] = blender::float4x4(working_to_display);
  filtered_inputs[12] = blender::float4x4(display_to_scene_linear);
  filtered_inputs[13] = log_midgray;
  filtered_inputs[14] = midgray;
  filtered_inputs[15] = blender::float4x4(insetmat);
  filtered_inputs[16] = blender::float4x4(outsetmat);
  filtered_inputs[17] = blender::float4x4(working_to_rec2020);
  filtered_inputs[18] = blender::float4x4(display_to_rec2020);
