...
static void node_update(bNodeTree *ntree, bNode *node)
{
  ...
  copy_m3_m3(data->scene_linear_to_working, (xyz_to_working * scene_to_xyz).ptr());
      ...
      return mf::build::detail::build_multi_function_with_n_inputs_one_output<float4>(
          ...
          [=, &builder, &data](const float4 &color,
            ...
            return agx_image_formation(
                ...
                float3x3(static_cast<const float*>(&data->scene_linear_to_working)),
                float3x3(static_cast<const float*>(&data->working_to_display)),
                float3x3(static_cast<const float*>(&data->display_to_scene_linear)),
                data->log_midgray,
                data->midgray,
                float3x3(static_cast<const float*>(&data->insetmat)),
                float3x3(static_cast<const float*>(&data->outsetmat)));
          },
