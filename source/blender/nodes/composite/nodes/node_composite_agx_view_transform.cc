    ...
    return mf::build::detail::build_multi_function_with_n_inputs_one_output<float4>(
      ...
      [=, &builder](const float4 &color,
        ...
        return agx_image_formation(
            ...
            pivot_offset_in,
            (builder.node().custom3 == int(AGXWorkingLog::AGX_WORKING_LOG_GENERIC_LOG2)) ? log2_min_in.value() : -10.0f,
            (builder.node().custom3 == int(AGXWorkingLog::AGX_WORKING_LOG_GENERIC_LOG2)) ? log2_max_in.value() : 6.5f,
            hue_flights_in,
            attenuation_rates_in,
            (builder.node().custom1) ? make_float3(0, 0, 0) : reverse_hue_flights_in.value(),
            (builder.node().custom1) ? make_float3(0, 0, 0) : restore_purity_in.value(),
